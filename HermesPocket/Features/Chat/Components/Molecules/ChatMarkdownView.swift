...
private enum Seg { case markdown(String); case math(String); case mermaid(String) }

private func segments(from src: String) -> [Seg] {
    var out: [Seg] = []
    let lines = src.components(separatedBy: "\n")
    var buf: [String] = []; var i = 0

    func flush() {
        let t = buf.joined(separator: "\n").trimmingCharacters(in: .whitespacesAndNewlines)
        buf.removeAll(); if !t.isEmpty { out.append(.markdown(t)) }
    }

    while i < lines.count {
        let t = lines[i].trimmingCharacters(in: .whitespaces)

        if let f = mermaidFence(t) {
            flush()
            var body: [String] = []; i += 1
            while i < lines.count, !lines[i].trimmingCharacters(in: .whitespaces).hasPrefix(f) {
...
private struct CodeBlockView: View {
    ...
    private var title: String {
        if let l = configuration.language?.trimmingCharacters(in: .whitespacesAndNewlines), !l.isEmpty { return l.uppercased() }
        return "CODE"
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack {
                Text(title).font(.caption.weight(.semibold)).foregroundStyle(.white.opacity(0.4))
                Spacer()
                Button {
            ...
            .padding(.horizontal, 12).padding(.vertical, 7).background(Color.white.opacity(0.06))

            ScrollView(.horizontal, showsIndicators: false) {
                Text(configuration.content.isEmpty ? " " : configuration.content)
                    .font(.system(size: 14, design: .monospaced))
                    .foregroundStyle(.primary).textSelection(.enabled).padding(12)
            }
        }
...
private struct MermaidView: View {
    let source: String; @State private var height: CGFloat = 200
    var body: some View {
...
private struct MermaidWebView: UIViewRepresentable {
    let source: String; @Binding var height: CGFloat
    func makeCoordinator() -> Coordinator { Coordinator(height: $height) }
    func makeUIView(context: Context) -> WKWebView {
        let c = WKWebViewConfiguration(); c.userContentController.add(context.coordinator, name: "resize")
        let v = WKWebView(frame: .zero, configuration: c)
        v.isOpaque = false; v.backgroundColor = .clear; v.scrollView.isScrollEnabled = false; v.scrollView.bounces = false
        v.loadHTMLString(mermaidHTML(source), baseURL: URL(string: "https://cdn.jsdelivr.net"))
        return v
    }
    func updateUIView(_ wv: WKWebView, context: Context) {
        if context.coordinator.last != source { context.coordinator.last = source; wv.loadHTMLString(mermaidHTML(source), baseURL: URL(string: "https://cdn.jsdelivr.net")) }
    }
    private func mermaidHTML(_ src: String) -> String {
        let e = src.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "`", with: "\\`")
        return "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1,maximum-scale=1\"><style>html,body{margin:0;padding:14px;background:transparent;overflow:hidden}svg{max-width:100%;height:auto}</style></head><body><div id=\"d\"></div><script src=\"https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.min.js\"></script><script>mermaid.initialize({startOnLoad:false,theme:'dark',securityLevel:'loose'});document.getElementById('d').innerHTML='<div class=\"mermaid\">\(e)</div>';mermaid.run({querySelector:'.mermaid'}).then(function(){window.webkit.messageHandlers.resize.postMessage(Math.max(document.getElementById('d').scrollHeight,120));});</script></body></html>"
    }
    final class Coordinator: NSObject, WKScriptMessageHandler {
        ...
        func userContentController(_: WKUserContentController, didReceive m: WKScriptMessage) {
            if m.name == "resize", let v = m.body as? Double { Task { @MainActor in height = max(120, v) } }
        }
    }
}
    ...
    @MainActor static let hermes = Theme()
        ...
        .taskListMarker { c in
            Image(systemName: c.isCompleted ? "checkmark.circle.fill" : "circle")
                .foregroundStyle(c.isCompleted ? .blue : .white.opacity(0.3)).font(.system(size: 20))
        }
        .table { c in
            ...
            }.markdownMargin(top: 18, bottom: 22).clipShape(RoundedRectangle(cornerRadius: 12))
            .overlay(RoundedRectangle(cornerRadius: 12).stroke(.white.opacity(0.12), lineWidth: 1))
        }
        .tableCell { c in
            c.label.markdownTextStyle { if c.row == 0 { FontWeight(.semibold); BackgroundColor(.white.opacity(0.06)) } else { BackgroundColor(.clear) } }
